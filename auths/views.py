...
class SignupView(APIView):
    ...
    def post(self, request):
        ...
        if user and user.is_email_verified==False:
            ...
            subject = 'Account Verification - SalonSeat'
            plain_message = f"""

Hello {user.full_name},
              
Thank you for registering with SalonSeat.
            ...
            try:
                ...
                if otp_object.otp == serializer.validated_data.get('otp'):
                    if timezone.now() - otp_object.updated_at <= timedelta(minutes=5):
                        ...
                        sent_note_to_user.delay(user_id=user.id, title="Congratulations!", content="Your email has been successfully verified. Welcome to SalonSeat", note_type='success')

                        return Response({"success":True,"message":"Your email has been successfully verified. Welcome to SalonSeat"}, status = status.HTTP_200_OK)
                    else:
